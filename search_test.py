    ...
    def display(self, screen, block_lt_pos, block_shape):
        if self.SOLVED is False:
            for k, v in self.close_list.items():
                pygame.draw.rect(screen, CORNFLOWERBLUE, (block_lt_pos[0], block_lt_pos[1], block_lt_pos[0] + v[0] * block_shape[0],
                                                          block_lt_pos[1] + v[1] * block_shape[1]))
        else:
            for i in self.path:
                pygame.draw.rect(screen, BLUEVIOLET, (block_lt_pos[0], block_lt_pos[1], block_lt_pos[0] + i[0] * block_shape[0],
...
def main():
    puzzle_map = PuzzleMap(7, 6, [1, 3], [5, 2], 'square', 1)
